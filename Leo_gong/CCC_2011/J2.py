...
wait for the weather balloon to return to ground. You can assume 0 ≤ h ≤ 100 and 0 < M <240.
...
"""
def balloon():
    file = open("CCC_2011/J2", "r")
    h = int(file.readline())
    max_t = int(file.readline())
    for t in range(1, max_t):
        A = -6 * t ** 4 + h * t ** 3 + 2 * t ** 2 + t
        if A <= 0:
            return "The balloon first touche the ground at hour: \n" + str(t)
