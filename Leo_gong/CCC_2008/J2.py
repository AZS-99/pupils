...
CCC music player (C^3MP) is currently in development and will be hitting the stores soon! In this problem, you have to
simulate a C^3MP.
...
def mp3():
    ...
    lst = ['A', 'B', 'C', 'D', 'E']
    while num != 4:
        button_num, num_pressess = int(input("Button number: ")), int(input("Number of presses: "))
        if button_num == 1:
            for i in range(num_pressess):
                letter = lst[0]
                lst.pop(0)
                lst.append(letter)
