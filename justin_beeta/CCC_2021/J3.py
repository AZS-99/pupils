...
"""
def professor_secret_formula():
    file = open("CCC_2021/J3", 'r')
    value = file.readline()
    last = str()
    while value != "99999":
        if (int(value[0]) + int(value[1])) % 2 == 0:
            print("right " + value[2] + value[3] + value[4])
            last = "right"
        elif(int(value[0]) + int(value[1])) % 2 == 1:
            print("left " + value[2] + value[3] + value[4])
            last = "left"
        else:
            print(last + value[2] + value[3] + value[4])
        value = file.readline()
