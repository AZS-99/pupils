...
it was a tie game.
"""
def basketball_game_between_apple_and_bananas():
    file = open("CCC_2019/J1", "r")
    three_point = int(file.readline())
    two_point = int(file.readline())
    one_point = int(file.readline())
    apples = three_point * 3 + two_point * 2 + one_point
    three_point = int(file.readline())
    two_point = int(file.readline())
    one_point = int(file.readline())
    bananas = three_point * 3 + two_point * 2 + one_point * 1
    if apples > bananas:
        return "A"
    elif bananas > apples:
        return "B"
