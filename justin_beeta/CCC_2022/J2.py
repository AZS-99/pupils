...
def star_players():
    ...
    players_num = int(file.readline()) # 3
    greater_than_40_count = 0          # 1
    for i in range(players_num):
        player_points = int(file.readline())
        player_fouls = int(file.readline())
        player_stars = player_points * 5 - player_fouls * 3
        if player_stars > 40:
            greater_than_40_count += 1
    print(greater_than_40_count, end="")
    if players_num == greater_than_40_count:
        print("+")
