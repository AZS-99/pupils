...
def tune():
    ...
    for i in range(len(input_line) - 1):
        if input_line[i].isdigit() and input_line[i+1].isalpha():
            indecies.append(i + 1)

    tunes = []
    last_index = 0
    for index in indecies:
        subdtring = input_line[last_index:index]
        last_index = index
        tunes.append(subdtring)
    tunes.append(input_line[last_index:])

    for chunk in tunes:
        if "+" in chunk:
            notes, spins = chunk.split("+")
            tweak = "tighten"
        else:
            notes, spins = chunk.split("-")
            tweak = "loosen"
        print(notes, tweak, spins)
