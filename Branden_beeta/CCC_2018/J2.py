...
def Parking_Lotter():
    ...
    lot_number = int(file.readline())
    yesterday = list(file.readline())
    today = list(file.readline())
    lots_occupied_both_days = 0
    for i in range(lot_number):
        if yesterday[i] == today[i] == 'C':
            lots_occupied_both_days += 1
    print(lots_occupied_both_days)
